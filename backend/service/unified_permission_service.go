...
func (s *unifiedPermissionService) CheckResourcePermission(userID uuid.UUID, resourceType string, resourceID uuid.UUID, action string) (bool, error) {
	// 获取用户的所有权限
	permissions, teamIDs, err := s.getUserPermissionsWithCache(userID)
	if err != nil {
		return false, err
	}
...
func (s *unifiedPermissionService) IsSuperAdmin(userID uuid.UUID) (bool, error) {
	permissions, _, err := s.getUserPermissionsWithCache(userID)
	if err != nil {
		return false, err
	}
...
func (s *unifiedPermissionService) getUserPermissionsWithCache(userID uuid.UUID) ([]*model.ScopedPermission, []uuid.UUID, error) {
	// 检查缓存
	s.cache.mu.RLock()
	if cached, ok := s.cache.data[userID]; ok && time.Now().Before(cached.expireAt) {
		s.cache.mu.RUnlock()
		return cached.permissions, cached.teamIDs, nil
	}
	s.cache.mu.RUnlock()
	...
	// 获取所有权限（用户直接权限 + 团队权限）
	permissions, err := s.permissionRepo.GetUserAllPermissions(userID, teamIDs, nil)
	if err != nil {
		return nil, nil, err
	}
	...
	return permissions, teamIDs, nil
}
